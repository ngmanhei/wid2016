...
import subprocess
import uuid
from wid2016_eval import wid2016_eval
...
class wid2016(imdb):
    ...
    def _load_image_set_index(self):
        ...
        image_set_file = os.path.join(self._data_path, 'list',
                                      self._image_set + '.list')
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'WID2016')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_wid2016_annotation(index)
    ...
    def _get_label_class(self):
        return ('__background__', 'TrafficSign')
        

    def _load_wid2016_annotation(self, index):
        """
        Load image and bounding boxes info from TXT file.
        """
        filename = os.path.join(self._data_path, 'labels', index + '.txt')
        # print 'Loading: {}'.format(filename)
        num_objs = len(open(filename, 'r').readlines())    # no background
    ...
    def _write_wid2016_results_file(self, all_boxes):
    ...
    def _do_matlab_eval(self, comp_id, output_dir='output'):
        ...
        path = os.path.join(cfg.ROOT_DIR, 'lib', 'datasets',
                            'WID2016devkit-matlab-wrapper')
        cmd = 'cd {} && '.format(path)
        cmd += '{:s} -nodisplay -nodesktop '.format(cfg.MATLAB)
        cmd += '-r "dbstop if error; '
        cmd += 'wid2016_eval(\'{:s}\',\'{:s}\',\'{:s}\',\'{:s}\'); quit;"' \
